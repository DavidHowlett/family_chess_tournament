...
v4.alpha_beta(
    difficultPosition, 3, v4.board_score(difficultPosition), True, -99999, 99999
)
v4.alpha_beta(
    promotionPosition, 3, v4.board_score(promotionPosition), True, -99999, 99999
)

# todo fix this bad move
bestMove = v4.search(
    list(v4.moves(kingSavePosition, True)),
    3,
    v4.board_score(kingSavePosition),
    True,
    -99999,
    99999,
)
...
def performance_test():
    ...
    best_move, _ = v4.search(
        _possible_moves,
        global_depth,
        v4.board_score(difficultPosition),
        True,
        -99999,
        99999,
    )
    print(
        "{}\t\t\t{}\t\t{:.3f}".format(v4.total_moves, global_depth, now() - start_time)
    )
    print(
