...
def match(white, black, repeat):
    ...
    for turn in range(1, 1 + turnsToPlayFor):
        ...
        white_time = (
            initialTime
            + white_moves * (timePerMove + (repeat - 1) * extraRepeatTime)
            - white_time_taken
        )
        black_time = (
            initialTime
            + black_moves * (timePerMove + (repeat - 1) * extraRepeatTime)
            - black_time_taken
        )
        try:
        ...
        if player_is_white:
            ...
            black_moves += 1
        print_state(
            turn, chosen_move, run_time, white_time, black_time, white, black, repeat
        )
    ...
    open(make_file_name(white, black, repeat), "w").write(
        current_versions + str(to_record)
    )
