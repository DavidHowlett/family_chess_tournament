...
assert (
    PIECE_VALUE["K"]
    > DISCOUNT_RATE * PIECE_VALUE["Q"] + DISCOUNT_RATE**2 * PIECE_VALUE["K"]
)
# for most pieces there is a small advantage to being in the centre
POSITION_VALUE = [
    [0.04 * (1 + x - x * x / 7) * (1 + y - y * y / 7) for x in range(8)]
    for y in range(8)
]
...
PAWN_POSITION_VALUE = [
    [0.003 * (10 + x - x * x / 6.9) * (y + 2) ** 2 for x in range(8)] for y in range(8)
]
                        ...
                        if target_piece == ".":
                            ...
                            _moves.append(
                                (
                                    ...
                                    position_multipler
                                    * (POSITION_VALUE[y2][x2] - POSITION_VALUE[y][x]),
                                )
                            )
                        elif (
                        ...
                        for replacement_piece in "QRBN" if _player_is_white else "qrbn":
                            ...
                            _moves.append(
                                (
                                    after_pawn_replacement,
                                    position_multipler
                                    * (POSITION_VALUE[y2][x] - POSITION_VALUE[y][x]),
                                )
                            )
                    else:
                        _moves.append(
...
def calculate_tree(state, depth):
    ...
    if children:
        if depth:
            ...
            state["score"] = DISCOUNT_RATE * (max if state["white"] else min)(
                child["diff"] + child["score"] for child in children
            )
        else:
            # then set the score to be the score diff of the best child (discounted for being in the future)
            state["score"] = DISCOUNT_RATE * (max if state["white"] else min)(
                child["diff"] for child in children
            )
    else:
...
def main(history, white_time, black_time):
    ...
    if global_depth > 1:
        final_state = (max if player_is_white else min)(
            possible_moves, key=lambda s: s["diff"] + s["score"]
        )
    elif global_depth == 1:
        final_state = (max if player_is_white else min)(
            possible_moves, key=lambda s: s["diff"]
        )
    print(global_depth)
