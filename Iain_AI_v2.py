...
for piece in list(Position_Scores):
    Position_Scores[piece.lower()] = [
        [-value for value in row] for row in Position_Scores[piece]
    ]
...
for piece in Position_Scores:
    Piece_score[piece] = [
        [value + PIECE_VALUE[piece] for value in row] for row in Position_Scores[piece]
    ]
...
def moves(board, white_turn):
    ...
    for y in range(8):
        for x in range(8):
            ...
            if white_turn:
                if piece in "KQRBN":
                    for y_move_direction, x_move_direction in PIECE_MOVE_DIRECTION[
                        piece
                    ]:
                                ...
                                if board[new_y][new_x] == ".":
                                    ...
                                    legal_moves.append(
                                        (
                                            ...
                                            Piece_score[piece][new_y][new_x]
                                            - Piece_score[piece][y][x]
                                            - Piece_score[board[new_y][new_x]][new_y][
                                                new_x
                                            ],
                                        )
                                    )
...
def search(board, white_turn, depth):
    ...
    best_score = (
        -99_900_000
    )  # this will only matter if there are no moves and avoids crashing runner
    best_move = board
    for move_, score_difference in moves(board, white_turn):
        # assert abs(score_difference + score_board - score(move_)) < 0.1
        current_score = (
            score_difference + score_board
            if white_turn
            else -(score_difference + score_board)
        )
